...
def main():
    model_ids = ["meta.llama3-70b-instruct-v1:0", "anthropic.claude-3-5-sonnet-20240620-v1:0"]
    ...
    for model_id in model_ids:
        ...
        response = agent.run()
        print(f"Base Agent Response for model {model_id}: {response}")
        save_output("data/case-study-25.json", response, tag=f"{model_id}-base")
    ...
    for model_id in model_ids:
        ...
        print(f"\nStored Agent Responses for model {model_id}:")
        for agent_name, response in mas.memory.items():
            print(f"Agent ID: {agent_name}\nResponse: {response}\n")
            if agent_name == "SYNTHESIS_AGENT":
                save_output("data/case-study-25.json", response, tag=f"{model_id}-synthesis")
